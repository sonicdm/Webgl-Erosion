...
import { domainwarp } from './terrain-noise-utils';

/**
 * Domain Warp terrain type (ID: 1)
 * Matches initial-frag.glsl: domainwarp(cpos * 2.0)
 */
export class DomainWarpTerrainType extends BaseTerrainType {
  getName(): string {
    return 'DomainWarp';
  }
  ...
  generateHeightmap(zs: Float32Array | number[], options: TerrainGenerationOptions): void {
    ...
    for (let y = 0; y < yl; y++) {
      for (let x = 0; x < xl; x++) {
        ...
        // Match shader exactly: terrain_height = domainwarp(cpos * 2.0);
        const terrain_height = domainwarp([cpos[0] * 2.0, cpos[1] * 2.0]);
