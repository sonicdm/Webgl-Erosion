...
import { voroni } from './terrain-noise-utils';

/**
 * Voronoi terrain type (ID: 3)
 * Matches initial-frag.glsl: voroni(cpos * 2.0) / 3.0
 */
export class VoronoiTerrainType extends BaseTerrainType {
  getName(): string {
    return 'Voronoi';
  }
  ...
  generateHeightmap(zs: Float32Array | number[], options: TerrainGenerationOptions): void {
    ...
    for (let y = 0; y < yl; y++) {
      for (let x = 0; x < xl; x++) {
        ...
        // Match shader exactly: terrain_height = voroni(cpos * 2.0) / 3.0;
        const terrain_height = voroni([cpos[0] * 2.0, cpos[1] * 2.0]) / 3.0;
